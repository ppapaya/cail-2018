...
class Predictor(object):
    def __init__(self):
        ...
        self.law = joblib.load('predictor/model/xgboost_law.model')
        self.accu = joblib.load('predictor/model/xgboost_accu.model')
        self.time = joblib.load('predictor/model/xgboost_time.model')
        self.batch_size = 1

        self.cut = thulac.thulac(seg_only = True)

    def predict_law(self, vec):
        y = self.law.predict(xgb.DMatrix(vec))
        yr = numpy.array(y)
        return yr + 1

    def predict_accu(self, vec):
        y = self.accu.predict(xgb.DMatrix(vec))
        yr = numpy.array(y)
        return yr + 1

    def predict_time(self, vec):

        y = self.time.predict(xgb.DMatrix(vec))[0]
